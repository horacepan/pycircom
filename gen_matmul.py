    ...
    for i, cs in row_vals.items():
        ...
        for j in range(n):
            ...
            for _ik in range(1, len(cs)):
                k = cs[_ik]
                cij.append(f"c{i}[{j}][{_ik}] <== c{i}[{j}][{_ik-1}] + A_vals[{idx + _ik}] * B[{k}][{j}]; // A_vals[{idx+_ik}] = A[{i}][{k}]") # A_vals[idx + _i] = A[i][k]
                # increment the index
            res += TAB + " ".join(cij) + "\n"
...
if __name__ == '__main__':
    ...
    rows = [1, 0, 1, 2, 1]
    cols = [1, 2, 0, 1, 1]
    data = np.random.randint(0, 100, (k,))

    A = csr_matrix((data, (rows, cols)), shape=(3,3))
    print("A", A.toarray())
    B = np.random.randint(0, high=30, size=(3, 3))
    js = {"A_vals": data, "B": B}

    print("B", B)
    print(A@B)
    #print(gen_matmul(A, B))
    print(gen_sparse_matmul(A, B))